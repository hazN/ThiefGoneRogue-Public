...
namespace RPG.Core.Editor
{
    ...
    public class PredicatePropertyDrawer : PropertyDrawer
    {
        ...
        {
            var inventoryItems = Resources.LoadAll<InventoryItem>("");
            item = (InventoryItem)EditorGUILayout.ObjectField("Select Inventory Item:", item, typeof(InventoryItem), false);
            if (item != null)
            {
                element.stringValue = item.GetItemID();
                item = null;
                return;
            }
